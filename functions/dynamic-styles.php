...
/* ------------------------------------ */
if ( ! function_exists( 'folias_hex2rgb' ) ) {

	function folias_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'folias_enqueue_google_fonts' ) ) {

	function folias_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'folias_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'folias_dynamic_css' ) ) {

	function folias_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = folias_hex2rgb($color_1);
...
.s2,
.s2 .social-links,
.toggle-search { background-color: '.esc_attr( get_theme_mod('color-social-sidebar') ).'; }
.toggle-search { border-bottom: 1px solid rgba(255,255,255,0.3); }
.s2 .social-links .social-tooltip:hover { background: rgba(0,0,0,0.1); }
.s2 .social-links .social-tooltip:active { color: rgba(255,255,255,0.7); }
				'."\n";
			}
			// content sidebar color
			if ( get_theme_mod('color-content-sidebar','#222222') != '#222222' ) {
				$styles .= '
...
.post-comments > span:before { border-color: transparent '.esc_attr( get_theme_mod('color-comments') ).' transparent transparent; }
			...
			wp_add_inline_style( 'folias-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'folias_dynamic_css' );
